...
with tabs[0]:
    ...
    if viz_type == "🏆 Perubahan Terbesar (17)":
        ...
        for sek in selected_sectors:
            ...
            anomalies = np.sum((data < lower) | (data > upper))
            if anomalies > 0:
                st.warning(f"⚠️ **{sek[:50]}**: Terdeteksi `{int(anomalies)}` periode anomali")
            else:
                st.success(f"✅ **{sek[:50]}**: Data stabil")
...
2. Buka terminal di folder yang sama, lalu jalankan:
   ```bash
   streamlit run app.py
   ```
3. Dashboard akan terbuka otomatis di browser (`http://localhost:8501`)

## 🌐 Deploy ke Streamlit Cloud (Gratis)
1. Buat repository GitHub baru, upload `app.py` dan dataset Excel Anda.
2. Pastikan dataset dapat diakses via **Raw URL** (format: `https://raw.githubusercontent.com/username/repo/branch/file.xlsx`)
3. Buka [share.streamlit.io](https://share.streamlit.io) → Hubungkan GitHub → Pilih repo & branch → Deploy.
4. Paste Raw URL di sidebar dashboard saat pertama kali membuka aplikasi.

## ✅ Fitur Utama
| Fitur | Keterangan |
|-------|------------|
| 📅 **Filter Periode** | Dropdown: Triwulan 1-4 & Tahunan |
| 🏷️ **Multi-Select Sektor** | Urutan persis seperti dataset asli (tanpa sorting) |
| 🏆 **Perubahan Terbesar (17)** | Bar horizontal 1 bar/sektor, ranking otomatis, warna hijau/merah |
| 📈 **Tren & Volatilitas** | Plotly interaktif dengan hover, zoom, dan download PNG |
| 🔍 **Deteksi Anomali** | IQR Method berbasis threshold slider |
| 📋 **Tabel Perubahan** | `st.dataframe()` dengan format `+xx.xx%` |
| 📥 **Export CSV** | Download data filter & ranking 17 sektor |
| 🌲 **Format Pohon** | `├─ ... └─ Tren:` dipertahankan di UI |

## 💡 Tips & Kustomisasi
- **Ganti Jumlah Ranking**: Ubah `df_growth.head(17)` menjadi `head(N)` sesuai kebutuhan.
- **Tambah Metode Anomali**: Integrasikan `zscore` atau `growth_spike` di blok `🔍 Deteksi Anomali`.
- **Auto-Refresh Data**: Cache TTL diatur `3600` detik (1 jam). Ubah nilai `@st.cache_data(ttl=...)` sesuai kebutuhan.
- **Security**: Untuk dashboard privat, tambahkan autentikasi sederhana atau deploy di organisasi tertutup.

---
📅 *Dibuat untuk Analisis Ekonomi & Monitoring PDB BPS Seri 2010* | 🛠️ *Python + Streamlit + Plotly*
